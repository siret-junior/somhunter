...
import { createShowGlobalNotification, createHideGlobalNotification } from "./notificationCreator";
...
export function createRescore(destDisplay, query0, query1) {
  ...
  return async (dispatch, getState) => {
    ...
    dispatch(createShowGlobalNotification(CS.GLOB_NOTIF_INFO, "Working...", "", 1000000))
    ...
    try {
      console.debug(
        "=> doRequestRescore: POST request to '/rescore'"
      );
...
export function createResetSearch(destDisplay) {
  return async (dispatch, getState) => {
    ...
    dispatch(createShowGlobalNotification(CS.GLOB_NOTIF_SUCC, "Search reset.", "", 2000))
