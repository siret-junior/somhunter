...
import CollageSearchPanel from "./CollageSearchPanel";
...
function MainPanel(props) {
  ...
  return (
    <Container fluid className="panel main p-0">
      <ControlsPanel>
        <Col xs={12} className="query-switches">
          <Form>
            <Form.Check
              inline
              type="switch"
              id="textQuerySwitch"
              label="Text"
              checked={config.ui.queries.text ? true : false}
              onChange={() => {
                config.ui.queries.text = !config.ui.queries.text;
            ...
            <Form.Check
              ...
              id="filtersSwitch"
              label="Filters"
              checked={config.ui.queries.filters ? true : false}
              onChange={() => {
                config.ui.queries.filters = !config.ui.queries.filters;
                setValue((value) => value + 1);
              }}
            />
            <Form.Check
              inline
              type="switch"
              id="collageQuerySwitch"
              label="Collage"
              checked={config.ui.queries.collage ? true : false}
              onChange={() => {
                config.ui.queries.collage = !config.ui.queries.collage;
                setValue((value) => value + 1);
              }}
            />
          </Form>
      ...
      <ControlsPanel>
        ...
        <Col xs={6} className="cont-btn">
          <Button
            ...
            variant="primary"
            onClick={() =>
              onTriggerRescoretHandler(settings, CS.DISP_TYPE_SOM, isAcOpen)
            }
          >
            Rescore to SOM
          </Button>
